...
public class BackPackerSendTravelItemAdapter extends RecyclerView.Adapter {
    private final Context context;
    private final LayoutInflater inflater;
    private List<TravelBean.RouteBean> list = new ArrayList<>();
    ...
    public BackPackerSendTravelItemAdapter(Context context, List<TravelBean.RouteBean> list){
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(position!=list.size()-1) {
            if(position==list.size()&& list.get(position-1).getRouteCountry()!=null) {
                ((NormalViewHolder) holder).country.setText(list.get(position-1).getRouteCountry());
                ((NormalViewHolder) holder).country.setTextSize(TypedValue.COMPLEX_UNIT_SP, 19);
            } else if(position!=list.size()&&list.get(position).getRouteCountry()!=null) {
                ((NormalViewHolder) holder).country.setText(list.get(position).getRouteCountry());
                ((NormalViewHolder) holder).country.setTextSize(TypedValue.COMPLEX_UNIT_SP, 19);
            }

            if(position==list.size()&& list.get(position-1).getRouteTime()!=null) {
                ((NormalViewHolder) holder).time.setText(list.get(position-1).getRouteTime());
                ((NormalViewHolder) holder).time.setTextSize(TypedValue.COMPLEX_UNIT_SP, 19);
            } else if(position!=list.size()&&list.get(position).getRouteTime()!=null) {
